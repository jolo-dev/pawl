---
editUrl: false
next: false
prev: false
title: "@pawl/cdk"
---

## Enumerations

- [AttributeType](/cdk/enumerations/attributetype/)
- [HttpLambdaResponseType](/cdk/enumerations/httplambdaresponsetype/)

## Classes

- [ApiDestination](/cdk/classes/apidestination/)
- [ApiGateway](/cdk/classes/apigateway/)
- [Authorization](/cdk/classes/authorization/)
- [Construct](/cdk/classes/construct/)
- [DynamoDbTableWithStreams](/cdk/classes/dynamodbtablewithstreams/)
- [EventBridge](/cdk/classes/eventbridge/)
- [HttpIamAuthorizer](/cdk/classes/httpiamauthorizer/)
- [HttpJwtAuthorizer](/cdk/classes/httpjwtauthorizer/)
- [HttpLambdaAuthorizer](/cdk/classes/httplambdaauthorizer/)
- [HttpUserPoolAuthorizer](/cdk/classes/httpuserpoolauthorizer/)
- [LambdaFunction](/cdk/classes/lambdafunction/)
- [LocalStack](/cdk/classes/localstack/)
- [SecretValue](/cdk/classes/secretvalue/)
- [Sqs](/cdk/classes/sqs/)
- [Stack](/cdk/classes/stack/)
- [Table](/cdk/classes/table/)
- [UserPool](/cdk/classes/userpool/)
- [UserPoolClient](/cdk/classes/userpoolclient/)

## Interfaces

- [ApiDestinationProps](/cdk/interfaces/apidestinationprops/)
- [ApiProps](/cdk/interfaces/apiprops/)
- [DynamoDbTableWithStreamsProps](/cdk/interfaces/dynamodbtablewithstreamsprops/)
- [EventSource](/cdk/interfaces/eventsource/)
- [EventTarget](/cdk/interfaces/eventtarget/)
- [LambdaProps](/cdk/interfaces/lambdaprops/)

## Type Aliases

- [BasicTagsProps](/cdk/type-aliases/basictagsprops/)
- [EventBridgeProps](/cdk/type-aliases/eventbridgeprops/)
- [SqsProps](/cdk/type-aliases/sqsprops/)

## Variables

- [BasicTags](/cdk/variables/basictags/)

## Functions

- [defineStacks](/cdk/functions/definestacks/)
- [Local](/cdk/functions/local/)
