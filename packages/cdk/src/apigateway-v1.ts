...
export class ApiGatewayV1 extends BasicConstruct {
  ...
  constructor(scope: Stack, id: string, props: ApiV1Props) {
    ...
    if (props.routes) {
      for (const [routeKey, target] of Object.entries(props.routes)) {
        this.route(routeKey as `${HttpMethod} /${string}`, target, props.authorizer, props.authorizationType);
      }
    }
  ...
  private route(
    ...
    resource.addMethod(method, new LambdaIntegration(target.lambda), methodOptions);
  }
