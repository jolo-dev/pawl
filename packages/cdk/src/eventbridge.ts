import { type ISource, Pipe } from "@aws-cdk/aws-pipes-alpha";
	...
	constructor(scope: Stack, id: string, props: EventBridgeProps) {
		...
		for (const target of props.targets) {
			if (target.type instanceof LambdaFunction) {
				this.createRule(
					"LambdaRule",
					new eventtarget.LambdaFunction(target.type.lambda, {
						deadLetterQueue: dlq,
					}),
					target.eventPattern,
				);
				...
				this.createRule(
					...
					new eventtarget.SqsQueue(target.type.queue, {
						messageGroupId: target.type.queue.fifo
							? `${Stack.of(this).stackName}-sqs`
							: undefined,
					}),
					target.eventPattern,
				);
