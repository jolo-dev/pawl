...
export class BedrockReviewModel implements ReviewModel {
	...
	async review(input: ReviewModelInput): Promise<ReviewModelResult> {
		...
		const baseMessages: NonNullable<ConverseCommandInput["messages"]> = [
		...
		this.#logger?.debug?.(
			"bedrock review: malformed output, requesting repair",
			{},
		);
	...
	async respond(input: CommentResponseInput): Promise<{
		reply: string;
...
function buildUserMessage(input: ReviewModelInput): string {
	...
	for (const file of input.changedFiles) {
		parts.push(
			`<file path="${escapeAttr(file.path)}" changeType="${file.changeType}">`,
		);
...
function buildReplyMessage(input: CommentResponseInput): string {
	...
	if (input.findings.length > 0) {
		...
		for (const finding of input.findings) {
			...
			parts.push(
				`    <recommendation>${escapeText(finding.recommendation)}</recommendation>`,
			);
			parts.push("  </finding>");
		}
		parts.push("</findings>");
	...
	if (input.conversation.length > 0) {
		...
		for (const turn of input.conversation) {
			const tag =
				turn.role === "reviewer" ? "reviewer-reply" : "untrusted-comment";
