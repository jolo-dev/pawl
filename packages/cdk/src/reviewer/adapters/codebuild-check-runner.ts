...
export class CodeBuildRuntimeTransport implements CodeBuildTransport {
  ...
  async getLogEvents(input: GetLogEventsCommandInput): Promise<GetLogEventsCommandOutput> {
...
export class CodeBuildCheckRunner implements CheckRunner {
  ...
  async run(input: CheckRunInput): Promise<CheckRunResult> {
    ...
    const startOutput = await this.#transport.startBuild({
      ...
      environmentVariablesOverride: [
        { name: "PAWL_CHECK_RUN_ID", type: "PLAINTEXT", value: this.#clock().toISOString() },
      ],
    });
  ...
  async #pollUntilTerminal(buildId: string): Promise<
    ...
    | {
        readonly status: "SUCCEEDED" | "FAILED" | "FAULT" | "STOPPED" | "TIMED_OUT";
...
export function generateBuildspec(
  ...
  for (const check of checks) {
    buildCommands.push(`      - echo "${CHECK_START_PREFIX}${check.name}:START>>>"`);
    buildCommands.push(
...
function parseCheckResults(
  ...
  for (const check of checks) {
    ...
    const { boundedLog, logTruncated } = boundAndScrub(checkLines.join("\n"), maxLogBytes);
    results.push({
      name: check.name,
...
export type { CheckRunInput, CheckRunResult, CheckResult, RequestRef, ReviewCycleSnapshot };
