...
import type { RequestRef, RevisionRange, ReviewRequest } from "../domain/review-request";
import type {
  ChangedFile,
  ChangedHunk,
  ChangedLine,
...
export interface CodeCommitClientPort {
  getPullRequest(repository: string, requestId: string): Promise<PullRequestSnapshot>;
  getDifferences(snapshot: PullRequestSnapshot): Promise<readonly CodeCommitChangedFile[]>;
  getFile(
    repository: string,
    commit: string,
    path: string,
    ...
    | undefined
  >;
  getComments(snapshot: PullRequestSnapshot): Promise<readonly CodeCommitReviewComment[]>;
  postComment(input: {
...
function mapChangeType(type: CodeCommitChangedFile["changeType"]): ChangedFile["changeType"] {
  return type === "ADDED" ? "added" : type === "DELETED" ? "deleted" : "modified";
}
...
export class CodeCommitProvider implements SourceControlProvider {
  ...
  constructor(options: CodeCommitProviderOptions = {}) {
    this.#client = options.client ?? new CodeCommitReviewClient(options.transport);
  ...
  async getRequest(ref: RequestRef): Promise<ReviewRequest> {
    ...
    return {
      ...
      status:
        snapshot.status === "MERGED" ? "merged" : snapshot.status === "CLOSED" ? "closed" : "open",
  ...
  async getDiff(ref: RequestRef, revisions: RevisionRange): Promise<readonly ChangedFile[]> {
    ...
    for (const difference of differences) {
      ...
      files.push({
        ...
        hunks: binary ? [] : makeHunks(before?.content ?? "", after?.content ?? ""),
      });
    }
    return files;
  }

  async getFile(ref: RequestRef, revision: string, path: string): Promise<string | undefined> {
    const file = await this.#client.getFile(ref.repository, revision, path);
    return file?.isBinary === true ? undefined : file?.content;
  }

  async listComments(ref: RequestRef, after?: string): Promise<readonly ReviewComment[]> {
  ...
  async #listComments(
    ...
    return comments
      .map((comment) => {
        const occurredAt = comment.createdAt ?? comment.updatedAt ?? this.#clock().toISOString();
        return {
  ...
  async postInlineFinding(
    ...
    const location =
      finding.location.kind === "line" ? this.#inlineLocation(files, finding) : undefined;
  ...
  async postStatusComment(
    ...
    return {
      ...
      contentHash: createHash("sha256").update(response.content, "utf8").digest("hex"),
    };
  }
  ...
  async reactToComment(ref: RequestRef, commentId: string, reaction: string): Promise<void> {
    // Request the snapshot so the provider validates the PR is accessible.
    await this.#snapshot(ref);
    await this.#client.putCommentReaction({ commentId, reactionValue: reaction });
  }
  ...
  #inlineLocation(
    ...
    const file = files.find(
      (entry) => entry.path === finding.path || entry.previousPath === finding.path,
    );
    const hunk = file?.hunks.find((entry) => entry.identity === candidate.hunkIdentity);
    if (
      file === undefined ||
      hunk === undefined ||
      !hunk.lines.some(
        (line) => line.changed && line.side === finding.side && line.line === candidate.line,
      )
    )
      return undefined;
    if (file.changeType === "deleted" && finding.side === "after") return undefined;
    if (file.changeType === "added" && finding.side === "before") return undefined;
    if (
  ...
  async #requestForRevisions(ref: RequestRef, revisions: RevisionRange): Promise<ReviewRequest> {
  ...
  async #snapshot(ref: RequestRef, revisions?: RevisionRange): Promise<PullRequestSnapshot> {
    const cached = this.#snapshots.get(this.#snapshotKey(ref));
    const base = cached ?? (await this.#client.getPullRequest(ref.repository, ref.requestId));
