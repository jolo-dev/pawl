...
const reactionValueSchema = z
  .string()
  .min(1)
  .max(128)
  .regex(/^\S+$/u, "reaction value must be a single emoji token");
    ...
    pullRequestTargets: z
      .array(
        z.object({
          ...
          mergeMetadata: z.object({ isMerged: z.boolean().optional() }).optional(),
        }),
      )
      .min(1),
  }),
...
const fileResponseSchema = z.object({
  ...
  fileContent: z.union([
    ...
    z.array(z.number().int().min(0).max(255)).transform((bytes) => Uint8Array.from(bytes)),
  ]),
});
...
const postCommentReplyInputSchema = z.object({
  inReplyTo: requiredString,
  content: z.string().min(1),
  clientRequestToken: requiredString,
});

const putCommentReactionInputSchema = z.object({
  commentId: requiredString,
  reactionValue: reactionValueSchema,
});
...
function mapComment(
  ...
  return {
    ...
    ...(comment.creationDate === undefined ? {} : { createdAt: comment.creationDate }),
    ...(comment.lastModifiedDate === undefined ? {} : { updatedAt: comment.lastModifiedDate }),
  };
}
...
export class CodeCommitReviewClient {
  ...
  async getPullRequest(
    ...
    return {
      ...
      status:
        target.mergeMetadata?.isMerged === true ? "MERGED" : response.pullRequest.pullRequestStatus,
  ...
  async getDifferences(snapshot: PullRequestSnapshot, maxResults = 100): Promise<ChangedFile[]> {
    ...
    for (let page = 0; page < MAX_PAGES; page += 1) {
      ...
      for (const difference of response.differences) {
        results.push({
          ...
          ...(difference.afterBlob === undefined ? {} : { after: mapBlob(difference.afterBlob) }),
        });
      }
  ...
  async getFile(repositoryName: string, commitId: string, filePath: string): Promise<FileBlob> {
  ...
  async getComments(snapshot: PullRequestSnapshot, maxResults = 100): Promise<ReviewComment[]> {
  ...
  async postCommentReply(input: {
    readonly inReplyTo: string;
    readonly content: string;
    readonly clientRequestToken: string;
  }): Promise<ReviewComment> {
    const parsed = postCommentReplyInputSchema.parse(input);
    const response = commentResponseSchema.parse(
      await this.#transport.send(
  ...
  async putCommentReaction(input: {
    readonly commentId: string;
    readonly reactionValue: string;
  }): Promise<void> {
    const parsed = putCommentReactionInputSchema.parse(input);
    await this.#transport.send(
      new PutCommentReactionCommand({
        commentId: parsed.commentId,
        reactionValue: parsed.reactionValue,
      }),
    );
  }
}
