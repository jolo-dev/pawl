import { reviewEventSchema, type ReviewEvent } from "../domain/review-event";
...
function matchesBot(value: string | undefined, options: CodeCommitEventFilterOptions): boolean {
...
function requestFrom(detail: RecordValue, envelope: RecordValue): RequestKey | undefined {
  const repository = firstString(
    detail.repositoryName,
    record(detail.requestParameters)?.repositoryName,
    Array.isArray(detail.repositoryNames) ? detail.repositoryNames[0] : undefined,
  ...
  const requestId = firstString(
    ...
    Array.isArray(parameters?.pullRequestIds) ? parameters?.pullRequestIds[0] : undefined,
  );
  if (repository === undefined || requestId === undefined) return undefined;
  return { provider: "codecommit", repository, requestId };
}
function validateNormalizedEvent(event: unknown): NormalizedCodeCommitEvent | undefined {
...
export function normalizeCodeCommitEvent(
  ...
  const directCloudTrail = !wrappedCloudTrail && string(envelope.eventSource) !== undefined;
  ...
  if (wrappedCloudTrail || directCloudTrail) {
    ...
    actor = firstString(record(detail.userIdentity)?.arn, payload.userIdentityArn, payload.author);
  ...
  if (id === undefined || at === undefined || request === undefined) return undefined;
  ...
  if (
    ...
    type = "human-comment";
    const inReplyTo = firstString(payload.inReplyTo, responseComment?.inReplyTo);
