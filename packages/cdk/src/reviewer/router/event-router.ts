...
export interface RecoveryInput
	extends Omit<LeaseRecoveryInput, "remoteStatus"> {
...
function failureFor(
	...
	return {
		...
		operation,
		reason:
			classifyRetryError(error) === "permanent"
				? "permanent-error"
				: "retry-exhausted",
		attempts,
		lastError: {
...
export class EventRouter {
		...
		try {
			...
			const preparedSnapshot =
				...
				(loadSnapshot === undefined
					? undefined
					: await this.#retrySnapshot(loadSnapshot));
	...
	async #start(
		...
		let result: RetryResult<unknown>;
		try {
		...
		if (arn === undefined)
			throw new Error("durable invocation returned no ARN");
		await this.#store.recordExecution(request, generation, arn);
		return arn;
	}
	...
	async #executionStatus(arn: string): Promise<RemoteExecutionStatus> {
		...
		if (status === undefined)
			throw new Error("durable execution returned an unknown status");
		return status;
	}
