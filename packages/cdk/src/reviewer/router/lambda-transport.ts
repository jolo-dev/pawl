...
export class AwsLambdaTransport implements LambdaTransport {
  ...
  send(command: LambdaCommand): Promise<unknown> {
    switch (command.kind) {
      ...
      case "list":
        return this.#client.send(new ListDurableExecutionsByFunctionCommand(command.input));
      case "status":
        return this.#client.send(new GetDurableExecutionCommand(command.input));
      case "callback":
        return this.#client.send(new SendDurableExecutionCallbackSuccessCommand(command.input));
    }
  }
}
