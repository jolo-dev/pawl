...
import type { AcceptedFinding, DismissalCandidate, FindingFingerprint } from "../domain/finding";
...
import type { ChangedFile, SourceControlProvider } from "../ports/source-control-provider";
import type { FindingWriteResult, PersistedFinding, ReviewStateStore } from "../ports/state-store";
...
export class IdempotentFindingReconciler implements FindingReconciler {
  ...
  async #postFinding(input: ReconcilerInput, finding: AcceptedFinding): Promise<void> {
    const fingerprint = createFindingFingerprint({
      ...
      ...(finding.location.kind === "line" ? { line: finding.location.line } : {}),
    });
    ...
    try {
      const posted = await this.#provider.postInlineFinding(input.request, finding, {
      ...
      const existing = await this.#findExistingProviderComment(input.request, finding);
  ...
  async #resolveFinding(input: ReconcilerInput, dismissal: DismissalCandidate): Promise<void> {
    const idempotencyToken = idempotencyTokenFor(input.request, dismissal.findingFingerprint);
    const reservation = await this.#store.reserveFindingWrite({
      operation: "resolve",
      request: input.request,
      generation: input.generation,
...
function nearbyCodeFor(finding: AcceptedFinding, files: readonly ChangedFile[]): readonly string[] {
  const file = files.find((f) => f.path === finding.path);
  if (file === undefined) return [];
  if (finding.location.kind !== "line") {
    const hunk = file.hunks.find((h) => h.identity === finding.location.hunkIdentity);
    return hunk ? hunk.lines.map((line) => line.content) : [];
  }
  const hunk = file.hunks.find((h) => h.identity === finding.location.hunkIdentity);
  if (hunk === undefined) return [];
  const line = finding.location.line;
  const matching = hunk.lines.find((l) => l.side === finding.side && l.line === line);
...
function idempotencyTokenFor(request: RequestKey, fingerprint: FindingFingerprint): string {
