...
function statusCode(error: Readonly<Record<string, unknown>>): number | undefined {
...
export function classifyRetryError(error: unknown): RetryClassification {
  ...
  if ((name && PERMANENT_NAMES.has(name)) || (code && PERMANENT_NAMES.has(code))) {
    return "permanent";
  }
  if ((name && RETRYABLE_NAMES.has(name)) || (code && RETRYABLE_CODES.has(code))) {
    return "retryable";
  }
...
function serializeError(error: unknown): SerializedOperationalError {
  ...
  const record = asRecord(error);
  return {
    name: record ? (stringField(record, "name") ?? "UnknownError") : "UnknownError",
    message: record ? (stringField(record, "message") ?? String(error)) : String(error),
  };
}
...
export class RetryPolicy {
  ...
  async execute<T>(operation: string, run: () => Promise<T> | T): Promise<RetryResult<T>> {
