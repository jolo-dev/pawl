...
import { modelReviewCandidateSchema } from "../domain/finding";
import { z } from "zod";
import type { AcceptedFinding, DismissalCandidate } from "../domain/finding";
...
import type { ChangedFile, ChangedLine, ReviewComment } from "../ports/source-control-provider";
import type { BlockedLimitDetail, PersistedFinding } from "../ports/state-store";
...
export class ReviewEngine {
  ...
  async review(input: ReviewEngineInput): Promise<ReviewEngineResult> {
    ...
    if (blocked !== undefined) return { status: "blocked", blockedLimit: blocked };
  ...
  async respond(
    ...
    const findingContext: CommentResponseInput["findings"] = findings.map((f) => ({
      severity: f.severity,
      category: f.category,
      path: f.path,
...
function checkHardLimits(
  ...
  if (diffBytes > limits.maxDiffBytes) {
    return { reason: "max-diff-bytes", observed: diffBytes, maximum: limits.maxDiffBytes };
  }
...
function chunkFiles(
  ...
  const perChunkBytes = Math.max(1, Math.floor((maxModelTokens / 2) * BYTES_PER_TOKEN));
  const chunks: ChangedFile[][] = [];
  let current: ChangedFile[] = [];
  let currentBytes = 0;
  for (const file of sorted) {
    const fileBytes = file.hunks.reduce(
      (sum, hunk) => sum + hunk.lines.reduce((s, line) => s + line.content.length, 0),
      0,
    );
...
function buildFindingPolicyContext(files: readonly ChangedFile[]): FindingPolicyContext {
  ...
  for (const file of files) {
    for (const hunk of file.hunks) {
      ...
      if (hunk.lines.some((line: ChangedLine) => line.changed && line.side === "after")) {
...
function buildDismissalPolicyContext(
  ...
  const map = new Map<string, { findingFingerprint: string; linkedProviderCommentId: string }>();
