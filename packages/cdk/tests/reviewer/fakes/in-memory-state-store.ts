...
function changedRecovery(
	state: StoredRequest | undefined,
): LeaseRecoveryResult {
	return {
		recovered: false,
		reason: "changed",
...
function recoveryEligible(
	...
	const terminal = ["COMPLETED", "TIMED_OUT", "FAILED"].includes(
	...
	return (
		!shouldStart &&
		hasPendingWork &&
		(terminal || (!hasCallback && state.leaseExpiresAt <= appendAt))
	);
}
...
export class InMemoryStateStore implements ReviewStateStore {
	...
	async claimEvents(
		...
		if (claimed.length > 0) {
			...
			if (claimed.length === 99 && hasRemainingEvents) {
				state.claimCursor = eventSortKey(lastClaimed.event);
			}
			const watermark = eventWatermark(lastClaimed.event);
			if (
	...
	async registerCallback(
		...
		state.blockedLimit =
			input.lifecycleState === "BLOCKED_LIMIT" ? input.blockedLimit : undefined;
	...
	async confirmFindingWrite(result: FindingWriteResult): Promise<void> {
		...
		if (reservation.write.operation === "post") {
			...
			state.findings.set(result.fingerprint, {
				...existing,
				status:
					reservation.write.resolution === "fixed" ? "resolved" : "dismissed",
				providerCommentId: result.providerCommentId,
				providerContentHash: result.providerContentHash,
				updatedAt: result.completedAt,
			});
	...
	async complete(
		...
		if (this.#hasPending(state) && reason.type !== "failed")
	...
	#leaseExpiry(heartbeat: string): string {
		return new Date(
			Date.parse(heartbeat) + this.#leaseDurationSeconds * 1_000,
		).toISOString();
	}
