			...
			hunks: [
				{
					...
					header: "@@ -1,2 +1,2 @@",
					lines: [
						{ side: "after", line: 1, content: "new line", changed: true },
					],
				},
			],
		},
...
describe("BedrockReviewModel", () => {
	...
	test("never logs prompt, diff, or comment content", async () => {
		...
		const spyLogger = {
			info: (m: string, d?: unknown) =>
				logged.push(`${m} ${JSON.stringify(d ?? {})}`),
	...
	test("respond() returns the model's reply text and usage", async () => {
		const transport = new FakeTransport([
			converseOutput("No, `eval` is unsafe here.", 15, 25),
		]);
		const model = new BedrockReviewModel({
			transport,
		...
		expect(transport.requests[0]?.system?.[0]?.text).toContain(
			"replying to a developer",
		);
	...
	test("respond() wraps human comments as untrusted data and includes findings", async () => {
		...
		const userText =
			(
				firstRequest?.messages?.[0]?.content?.[0] as
					| { text?: string }
					| undefined
			)?.text ?? "";
