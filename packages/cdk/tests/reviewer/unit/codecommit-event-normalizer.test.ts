...
test("normalizes native request, revision, comment, and closed events without bodies", () => {
	...
	expect(JSON.stringify(normalizeCodeCommitEvent(base))).not.toContain(
		"must-not-persist",
	);
	expect(
		normalizeCodeCommitEvent({
...
test("filters reviewer and bot events and retains provider event IDs", () => {
	...
	expect(
		normalizeCodeCommitEvent(event, { reviewerArn: "arn:bot" }),
	).toBeUndefined();
	expect(
		normalizeCodeCommitEvent(event, { botArnPatterns: ["arn:bot"] }),
	).toBeUndefined();
	expect(
		normalizeCodeCommitEvent({
			...event,
...
test("applies global bot patterns deterministically across repeated events", () => {
	...
	expect(
		normalizeCodeCommitEvent(event, { botArnPatterns: [botPattern] }),
	).toBeUndefined();
	expect(
		normalizeCodeCommitEvent(event, { botArnPatterns: [botPattern] }),
	).toBeUndefined();
...
test("normalizes wrapped CloudTrail comments and filters their nested actor", () => {
	...
	expect(JSON.stringify(normalizeCodeCommitEvent(event))).not.toContain(
		"secret body",
	);
	expect(
		normalizeCodeCommitEvent(event, { reviewerArn: "arn:human" }),
	).toBeUndefined();
	expect(
		normalizeCodeCommitEvent(event, { botArnPatterns: [/arn:human/g] }),
	).toBeUndefined();
});
