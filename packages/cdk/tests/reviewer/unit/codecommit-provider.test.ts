  ...
  const client = {
    ...
    getFile: async (_repo: string, revision: string) => {
      ...
      return {
        ...
        fileSize: 4,
        isBinary: false,
        content: revision === "destination-commit-123" ? "old\nline\n" : "old\nnew\n",
      };
    },
    getComments: async () => [],
    postComment: async () => ({
    ...
    postCommentReply: async () => ({ commentId: "r", content: "r", authorArn: "reviewer" }),
...
test("computes deterministic multi-hunk line positions without double advancing context", async () => {
  ...
  const changed = files[0]?.hunks.flatMap((hunk) => hunk.lines.filter((line) => line.changed));
...
test("accepts only the existing side of renamed paths for inline comments", async () => {
  ...
  const hunkIdentity = files[0]!.hunks[0]!.identity;
  const base = finding() as Finding;
  await provider.postInlineFinding({ provider: "codecommit", repository: "repo", requestId: "7" }, {
    ...base,
    path: "old.ts",
    side: "before",
  ...
  await expect(
    provider.postInlineFinding({ provider: "codecommit", repository: "repo", requestId: "7" }, {
      ...base,
      path: "old.ts",
      side: "after",
      location: { kind: "line", line: 1, hunkIdentity },
...
test("filters and orders comments after the supplied watermark", async () => {
  ...
  const provider = new CodeCommitProvider({ client });
  const comments = await provider.listComments(refForTest(), "2026-01-01T00:00:01.000Z#before");
...
test("reactToComment forwards an emoji reaction to the client", async () => {
  ...
  const client = {
    ...
    getFile: async () => undefined,
    getComments: async () => [],
    postComment: async () => ({ commentId: "c", content: "x", authorArn: "reviewer" }),
    updateComment: async () => ({ commentId: "c", content: "x", authorArn: "reviewer" }),
    postCommentReply: async () => ({ commentId: "r", content: "r", authorArn: "reviewer" }),
    putCommentReaction: async (input: { commentId: string; reactionValue: string }) => {
      calls.push(input);
    },
  };
  const provider = new CodeCommitProvider({ client });
...
test("replyToComment posts a threaded reply with an idempotency token", async () => {
  ...
  const client = {
    ...
    postCommentReply: async (input: {
      ...
      calls.push(input);
      return { commentId: "reply-1", content: input.content, authorArn: "reviewer" };
    },
    putCommentReaction: async () => {},
  };
  const provider = new CodeCommitProvider({ client });
