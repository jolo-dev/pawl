...
describe("CodeCommitReviewClient", () => {
    ...
    const sender = new FakeSender([
      {
        pullRequest: {
          ...
          pullRequestTargets: [{ ...target, mergeMetadata: { isMerged: "yes" } }],
        },
      },
    ]);
    const client = new CodeCommitReviewClient(sender);
  ...
  test("paginates differences with NextToken and MaxResults", async () => {
    ...
    expect(files.map(({ changeType }) => changeType)).toEqual(["MODIFIED", "ADDED"]);
  });
  ...
  test("rejects cyclic differences pagination tokens", async () => {
    ...
    await expect(client.getDifferences(snapshot)).rejects.toThrow("pagination repeated a token");
  ...
  test("stops differences pagination at the hard page bound", async () => {
    ...
    await expect(client.getDifferences(snapshot)).rejects.toThrow("exceeded 1000 pages");
  ...
  test("retains the differences page-size limit", async () => {
    ...
    expect((sender.commands[0] as GetDifferencesCommand).input.MaxResults).toBe(100);
  ...
  test("always sends an explicit commit ID when reading a file", async () => {
    ...
    const file = await client.getFile("widgets", "source-immutable-abc", "src/a.ts");
  ...
  test("rejects a file returned from an unexpected commit", async () => {
    const sender = new FakeSender([{ ...fileFixture, commitId: "different-commit" }]);
    const client = new CodeCommitReviewClient(sender);

    await expect(client.getFile("widgets", "source-immutable-abc", "src/a.ts")).rejects.toThrow(
      "unexpected commit",
    );
  });
  ...
  test("classifies binary files without returning decoded content", async () => {
    ...
    const file = await client.getFile("widgets", "source-immutable-abc", "assets/image.bin");
  ...
  test("classifies invalid UTF-8 without NUL bytes as binary", async () => {
    const sender = new FakeSender([{ ...fileFixture, fileSize: 2, fileContent: [195, 40] }]);
    const client = new CodeCommitReviewClient(sender);
  ...
  test("paginates comments and preserves reply, author, and location", async () => {
    ...
    expect((sender.commands[0] as GetCommentsForPullRequestCommand).input).toEqual({
      repositoryName: "widgets",
      pullRequestId: "42",
    ...
    expect((sender.commands[1] as GetCommentsForPullRequestCommand).input).toEqual({
      repositoryName: "widgets",
      pullRequestId: "42",
      beforeCommitId: "destination-immutable-def",
      afterCommitId: "source-immutable-abc",
  ...
  test("accepts the comments page-size limit and rejects values above it", async () => {
    ...
    expect((sender.commands[0] as GetCommentsForPullRequestCommand).input.maxResults).toBe(500);
  ...
  test("rejects cyclic comments pagination tokens", async () => {
    ...
    await expect(client.getComments(snapshot)).rejects.toThrow("pagination repeated a token");
    expect(sender.commands).toHaveLength(3);
  });
  ...
  test("stops comments pagination at the hard page bound", async () => {
    ...
    await expect(client.getComments(snapshot)).rejects.toThrow("exceeded 1000 pages");
    expect(sender.commands).toHaveLength(1_000);
  });
  ...
  test("posts against exact cycle commits with a stable request token", async () => {
    ...
    expect((sender.commands[0] as PostCommentForPullRequestCommand).input).toEqual({
      repositoryName: "widgets",
      pullRequestId: "42",
      beforeCommitId: "destination-immutable-def",
      afterCommitId: "source-immutable-abc",
