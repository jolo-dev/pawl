...
const dismissal = {
	...
	rationale:
		"The linked context identifies an invariant enforced before this call.",
} as const;
...
describe("finding schemas and policy", () => {
	...
	it("requires the high-confidence threshold", () => {
		expect(evaluateFindingCandidate(finding, findingContext).accepted).toBe(
			true,
		);
		expect(
			evaluateFindingCandidate(
	...
	it("accepts only an exact trusted directly introduced hunk", () => {
		...
		expect(evaluateFindingCandidate(hunkFinding, findingContext).accepted).toBe(
			true,
		);
		expect(
			evaluateFindingCandidate(
	...
	it("requires all dismissal fields to match one trusted linkage tuple", () => {
		expect(
			evaluateDismissalCandidate(dismissal, dismissalContext).accepted,
		).toBe(true);
		expect(
			evaluateDismissalCandidate(
				{
