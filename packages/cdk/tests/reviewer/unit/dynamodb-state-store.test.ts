...
import type { Finding, FindingFingerprint } from "../../../src/reviewer/domain/finding";
import {
  DynamoDbStateStore,
...
describe("transactional state protocol", () => {
  ...
  it("enforces completion lifecycle ownership and rejects repeat completion", async () => {
    ...
    await expect(store.complete(request, 1, { type: "clean" })).rejects.toBeInstanceOf(
      StaleStateError,
    );
    await store.claimEvents(request, 1);
    await expect(store.complete(request, 1, { type: "clean" })).resolves.toBeUndefined();
    await expect(store.complete(request, 1, { type: "clean" })).rejects.toBeInstanceOf(
      StaleStateError,
    );
  });
  ...
  it("does not complete while pending work exists and eventually claims 105 events", async () => {
    ...
    await expect(store.complete(request, 1, { type: "clean" })).resolves.toBeUndefined();
    await expect(store.appendEvent(revisionEvent("after-complete"))).resolves.toMatchObject({
      generation: 2,
      shouldStart: true,
    });
  });
  ...
  it("appends before returning a callback to wake", async () => {
    ...
    expect((await store.claimEvents(request, 1)).events.map(({ id }) => id)).toEqual(["push-2"]);
  });
  ...
  it("detects an event between callback persistence and the final inbox check", async () => {
    ...
    store = new InMemoryStateStore({
      ...
      afterCallbackPersisted: async () => {
        await store.appendEvent(revisionEvent("push-raced", "2026-07-18T12:01:00.000Z"));
      },
    });
    await store.appendEvent(revisionEvent("push-1"));
    await store.recordExecution(request, 1, "arn:execution:1");
    ...
    expect((await store.claimEvents(request, 1)).events.map(({ id }) => id)).toEqual([
      "push-raced",
    ]);
    expect((await store.claimEvents(request, 1)).events).toEqual([]);
  });
  ...
  it("reserves and conditionally confirms finding writes", async () => {
    ...
    if (!resolution.reserved) throw new Error("expected resolution reservation");
    await store.confirmFindingWrite({
      request,
      generation: 1,
  ...
  it("recovers an expired WAITING owner and preserves findings on BLOCKED_LIMIT", async () => {
    ...
    await store.appendEvent(revisionEvent("push-2", "2026-07-18T12:01:00.000Z"));
    clock.value = new Date("2026-07-18T12:02:00.000Z");
  ...
  it("bounds fake keys for maximum schema-valid UTF-8 identifiers", async () => {
    ...
    const inspected = [firstRequest, secondRequest].map((key) => store.inspectRequest(key));
    expect(new Set(inspected.map((state) => state?.partitionKey)).size).toBe(2);
    for (const state of inspected) {
      expect(Buffer.byteLength(state!.partitionKey, "utf8")).toBeLessThanOrEqual(2048);
      expect(state!.eventSortKeys).toHaveLength(1);
      expect(state!.eventSortKeys[0]).toMatch(
        /^EVENT#2026-07-18T10:00:00\.000Z#v1~[A-Za-z0-9_-]{43}$/,
      );
      expect(Buffer.byteLength(state!.eventSortKeys[0]!, "utf8")).toBeLessThanOrEqual(1024);
    }
  });
  ...
  it("uses collision-safe request keys and canonical event instants", async () => {
    ...
    await store.appendEvent(revisionEvent("equal-b", "2026-07-18T10:00:00.000Z"));
    await store.appendEvent(revisionEvent("equal-a", "2026-07-18T12:00:00.000+02:00"));
    await store.appendEvent(revisionEvent("later", "2026-07-18T11:00:00.000Z"));
    const claimed = await store.claimEvents(request, 1);
    expect(claimed.events.map(({ id }) => id)).toEqual(["equal-a", "equal-b", "later"]);
...
describe("DynamoDbStateStore", () => {
  it("uses the REQUEST single-table keys, transactions, and deterministic TTL", async () => {
    ...
    ) as { input?: { TransactItems?: readonly Record<string, unknown>[] } } | undefined;
  ...
  it("conditions FAILED completion on exact callback and lease ownership", async () => {
    ...
    const callbackCommand = JSON.stringify((commands.at(-1) as { input?: unknown }).input);
    expect(callbackCommand).toContain("leaseVersion = :leaseVersion");
    expect(callbackCommand).toContain("callbackId = :callbackId");
    expect(callbackCommand).toContain("callbackGeneration = :callbackGeneration");
    ...
    const leaseCommand = JSON.stringify((commands.at(-1) as { input?: unknown }).input);
  ...
  it("conditions adapter completion on zero pending events and generation", async () => {
    ...
    const serialized = JSON.stringify((commands[0] as { input?: unknown }).input);
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        if (serialized.includes("reservationId")) {
          const token = serialized.includes("same-token") ? "same-token" : "other-token";
  ...
  it("uses the scanned hashed sort-key boundary rather than sorted provider-event output", async () => {
    const ids = Array.from({ length: 100 }, (_, index) => `provider-event-${index}`);
    const scanned = ids
      .map((id) => ({
        id,
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        if (command.constructor.name === "QueryCommand") {
          ...
          if (queryCount === 1) {
            ...
            return {
              ...
              LastEvaluatedKey: { pk: "request", sk: remaining[0]!.sk },
            };
          }
        ...
        if (command.constructor.name === "TransactWriteCommand") {
          const serialized = JSON.stringify((command as { input?: unknown }).input);
  ...
  it("invalidates a stale cursor when an out-of-order event is appended", async () => {
    ...
    for (let index = 0; index < 100; index += 1) {
      await store.appendEvent(
        revisionEvent(`cursor-${index}`, new Date(Date.parse(now) + index * 1_000).toISOString()),
      );
    }
    await store.claimEvents(request, 1);
    expect(store.inspectRequest(request)?.claimCursor).toBeDefined();

    await store.appendEvent(revisionEvent("late-arrival", "2026-07-18T11:00:00.000Z"));
  ...
  it("preserves the last claimed item as the cursor at the 99-event page boundary", async () => {
    ...
    const store = new DynamoDbStateStore({ transport, tableName: "state" });
    expect((await store.claimEvents(request, 1)).events).toHaveLength(99);
    expect((await store.claimEvents(request, 1)).events.map(({ id }) => id)).toEqual(["event-99"]);
  });
  ...
  it("rejects a stale cursor claim after an append invalidates the cursor", async () => {
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        if (command.constructor.name === "QueryCommand") {
          const input = (command as { input?: { ExclusiveStartKey?: { sk?: string } } }).input;
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        if (command.constructor.name === "TransactWriteCommand") {
          transactionCount += 1;
          const serialized = JSON.stringify((command as { input?: unknown }).input);
          expect(serialized).toContain("pendingEventCount = :observedPendingCount");
          if (transactionCount === 1) {
            throw {
              name: "TransactionCanceledException",
              CancellationReasons: [{ Code: "ConditionalCheckFailed" }],
  ...
  it("retries cursor-only metadata updates after an append race", async () => {
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        if (command.constructor.name === "TransactWriteCommand") {
          transactionCount += 1;
          expect(JSON.stringify((command as { input?: unknown }).input)).toContain(
  ...
  it("continues claiming after more than four retained pages", async () => {
    ...
    expect((await store.claimEvents(request, 1)).events.map(({ id }) => id)).toEqual(["later"]);
  ...
  it("encodes request-key components and canonicalizes event timestamps", async () => {
    ...
    const serialized = commands
      .filter(({ constructor }) => constructor.name === "TransactWriteCommand")
      .map((command) => JSON.stringify((command as { input?: unknown }).input));
    const partitionKeys = serialized.map((value) => value.match(/REQUEST#[^"]+/)?.[0] ?? "");
    expect(new Set(partitionKeys).size).toBe(3);
    for (const partitionKey of partitionKeys) {
      expect(partitionKey).toMatch(
        /^REQUEST#v1~[A-Za-z0-9_-]{43}#v1~[A-Za-z0-9_-]{43}#v1~[A-Za-z0-9_-]{43}$/,
    ...
    expect(serialized[0]).toMatch(/EVENT#2026-07-18T10:00:00.000Z#v1~[A-Za-z0-9_-]{43}/);
  });
  ...
  it("bounds maximum UTF-8 request and event identifiers in every generated key", async () => {
    ...
    const transport: DynamoDbDocumentTransport = {
      send: async (command) => {
        commands.push(command);
        if (command.constructor.name !== "GetCommand") return {};
        const sk = (command as { input?: { Key?: { sk?: string } } }).input?.Key?.sk;
    ...
    const serialized = transactions.map((command) => JSON.stringify(command.input));
    const keys = serialized.flatMap((value) =>
      [...value.matchAll(/"(?:pk|sk)":"([^"]+)"/g)].map((match) => match[1]!),
    );
    ...
    for (const key of eventKeys) {
      expect(key).toMatch(/^EVENT#2026-07-18T10:00:00\.000Z#v1~[A-Za-z0-9_-]{43}$/);
      expect(Buffer.byteLength(key, "utf8")).toBeLessThanOrEqual(1024);
    }
    expect(findingKeys).toHaveLength(1);
    expect(findingKeys[0]).toBe(`FINDING#${fingerprint}`);
    expect(Buffer.byteLength(findingKeys[0]!, "utf8")).toBeLessThanOrEqual(1024);
  ...
  it("only treats retained conditional cancellation reasons as contention", async () => {
    ...
    const store = new DynamoDbStateStore({ transport, tableName: "state" });
    await expect(store.appendEvent(revisionEvent("conditional"))).resolves.toMatchObject({
      appended: false,
    });
  });
  ...
  it("rethrows unsafe transaction cancellation shapes from append", async () => {
    ...
    for (const error of unsafe) {
      ...
      const store = new DynamoDbStateStore({ transport, tableName: "state" });
      await expect(store.appendEvent(revisionEvent(error.message))).rejects.toBe(error);
    }
  });
  ...
  it("rethrows transaction conflicts from reserve and recovery", async () => {
    ...
    const reserveTransport: DynamoDbDocumentTransport = {
      send: async (command) => {
        ...
        gets += 1;
        return gets === 1 ? { Item: { lifecycleState: "RUNNING", generation: 1 } } : {};
      },
    };
