...
import type { AcceptedFinding, DismissalCandidate } from "../../../src/reviewer/domain/finding";
...
const request: RequestKey = { provider: "codecommit", repository: "repo", requestId: "7" };
...
interface FakeProviderCalls {
  ...
  markCommentResolved: Array<{ ref: RequestKey; comment: PostedComment; resolution: Resolution }>;
  listComments: number;
}
...
class FakeProvider implements SourceControlProvider {
  ...
  constructor(options: { throwOnNextPost?: boolean; existingComments?: ReviewComment[] } = {}) {
    this.#throwOnNextPost = options.throwOnNextPost ?? false;
    if (options.existingComments) this.#existingComments.push(...options.existingComments);
  }
  ...
  async postInlineFinding(ref: RequestKey, finding: AcceptedFinding): Promise<PostedComment> {
...
describe("IdempotentFindingReconciler", () => {
  test("posts a new accepted finding and confirms it in state", async () => {
    ...
    const reconciler = new IdempotentFindingReconciler({ store, provider, clock: fixedClock });
  ...
  test("resolves an open finding via a linked dismissal candidate", async () => {
    ...
    const fingerprint = posted[0]!.fingerprint;
    const providerCommentId = posted[0]!.providerCommentId!;
  ...
  test("leaves carry-forward open findings untouched (no auto-resolve)", async () => {
    ...
    await reconciler.apply(
      baseInput({ candidates: [], existingFindings: posted as PersistedFinding[] }),
    );
  ...
  test("re-throws when the provider errors and no existing comment matches", async () => {
    ...
    await expect(reconciler.apply(baseInput({ candidates: [finding()] }))).rejects.toThrow(
      "network timeout",
    );
