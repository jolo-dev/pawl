...
describe("RetryPolicy", () => {
  ...
  it("keeps full jitter between zero and the bounded maximum", () => {
    expect(new RetryPolicy({ ...options, random: () => 0 }).delayForAttempt(20)).toBe(0);
    expect(new RetryPolicy({ ...options, random: () => 0.999 }).delayForAttempt(20)).toBeLessThan(
      options.maxDelayMs,
    );
  });

  it.each([Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY, -0.01, 1])(
    "rejects an invalid random result %p",
    (randomResult) => {
