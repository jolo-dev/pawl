...
class StubModel implements ReviewModel {
  ...
  async review(_input: ReviewModelInput): Promise<ReviewModelResult> {
    return { output: this.output, modelId: "stub", usage: { inputTokens: 5, outputTokens: 7 } };
  }
  async respond(): Promise<{
    reply: string;
    usage: { inputTokens: number; outputTokens: number };
  }> {
    return { reply: "✅ Reviewed.", usage: { inputTokens: 0, outputTokens: 0 } };
  }
}
...
describe("ReviewEngine", () => {
  ...
  test("drops a high-confidence finding on an untrusted line", async () => {
    const model = new StubModel({
      candidates: [finding({ location: { kind: "line", line: 999, hunkIdentity: "hunk-1" } })],
    });
    const engine = new ReviewEngine({ model });
      ...
      hunks: [
        {
          identity: "h",
          header: "@@ -1,1 +1,1 @@",
          lines: [{ side: "after", line: 1, content: "x".repeat(2_000_000), changed: true }],
        },
      ],
    };
      ...
      hunks: [
        {
          identity: "h",
          header: "@@ -1,1 +1,1 @@",
          lines: [{ side: "after", line: 1, content: "x".repeat(500_000), changed: true }],
        },
      ],
    };
      ...
      hunks: [
        {
          identity: "hunk-1",
          header: "@@ -1,1 +1,1 @@",
          lines: [{ side: "after" as const, line: 1, content: "x".repeat(300), changed: true }],
        },
      ],
    }));
  ...
  test("respond() returns the model reply when the model succeeds", async () => {
    const model: ReviewModel = {
      ...
      async respond() {
        return { reply: "No, eval is unsafe.", usage: { inputTokens: 3, outputTokens: 5 } };
      },
    };
    const engine = new ReviewEngine({ model });
    const { reply, usage } = await engine.respond(
      { snapshot, changedFiles: [changedFile], checks: [], humanComments: [], conversation: [] },
      [],
