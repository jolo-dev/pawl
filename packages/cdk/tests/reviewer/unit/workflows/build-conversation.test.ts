...
describe("buildConversation", () => {
  test("keeps human comments and reviewer replies, drops inline findings, orders by input", () => {
    const comments: ReviewComment[] = [
      ...
      comment({ id: "r1", body: `No, eval is unsafe here.\n---\n${SIG}`, inReplyTo: "h1" }),
    ...
    expect(turns).toEqual([
      ...
      { role: "reviewer", id: "r1", body: "No, eval is unsafe here.", inReplyTo: "h1" },
      { role: "human", id: "h2", body: "What about JSON.parse?", inReplyTo: "r1" },
    ]);
  });

  test("strips the 🤖 signature from reviewer reply bodies", () => {
    const turns = buildConversation([comment({ id: "r1", body: `Answer.\n---\n${SIG}` })], NAME);
