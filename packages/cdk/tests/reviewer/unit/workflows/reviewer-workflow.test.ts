...
import { ReviewerWorkflow, type ReviewerEvent } from "../../../../src/reviewer/workflows/reviewer-workflow";
...
const fakeProvider = {
  getRequest: async (): Promise<ReviewRequest> => ({ ...fakeReviewRequest, status: prStatus }),
...
function createWorkflow(store: InMemoryStateStore): ReviewerWorkflow {
  prStatus = "open";
...
async function seedRunning(store: InMemoryStateStore): Promise<void> {
  ...
  await store.recordExecution(request, 1, "arn:aws:lambda:us-east-1:123456789012:execution-1");
}
...
describe("ReviewerWorkflow", () => {
  ...
  test("happy path: begin → claim → review → register callback → wait → wake", async () => {
    ...
    const handler = withDurableExecution<ReviewerEvent, void>(async (event, context) => {
  ...
  test("stale callback: workflow returns when the callback is cleared (merged)", async () => {
    ...
    expect(status !== undefined && ["SUCCEEDED", "FAILED"].includes(status)).toBe(true);
    const state = store.inspectRequest(request);
