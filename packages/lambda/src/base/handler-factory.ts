...
export function handlerFactory<AwsEventType, AwsResultType = void>(
  serviceName: string,
  handleRequest: (event: AwsEventType, logger: Logger) => Promise<AwsResultType>,
  options?: {
    ...
    try {
      ...
      for (const hook of errorHooks) {
        try {
          ...
          // biome-ignore lint/correctness/noUnnecessaryContinue: // Try next error hook if this one fails
          continue;
        }
      }
